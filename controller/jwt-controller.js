...
export const authenticateToken = (request, response, next) => {
    ...
    // console.log("token is stored in headers - authorization",authHeader);
    const token = authHeader && authHeader.split(' ')[1];
    // console.log(token)
    
    
    if(token == null){
        return response.status(401).json({msg:"token is missing- NULL token"});
    }

    jwt.verify(token,process.env.ACCESS_SECRECT_KEY, (error,user) => {
        if(error){
